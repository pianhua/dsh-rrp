...
| **聊天流插入位**（**已裁决不用**） | `conversation.chat.turnTail` / `assistant-actions` / `chat.node`（keyed，`ChatNodeDataMap` 可声明合并） | `@deepseek-ai/dsh-client-ui-chat` | 官方确有正文插入位；D21 已知情否决——聊天流内不放插件内容，界面走舞台页签 |
