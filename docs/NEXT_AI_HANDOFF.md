...
本工作区尚未提交；不要 `reset`、`checkout`、清空或覆盖现有 diff。它包含本轮架构收敛、文档同步和对应测试，均属于待交付工作树。
...
本轮最后记录：`pnpm test` 21 文件 / 117 用例通过，`pnpm run typecheck`、`pnpm run build`、`git diff --check` 通过。后续验证已经移交外部测试者；下一位 AI 不应把历史结果当作新改动的证据，应根据测试者反馈做最小修复。

真实宿主验收仍待完成，见 [`reference/MANUAL_TEST.md`](reference/MANUAL_TEST.md) §8-§10。只能使用 `rp-dev:3099`，绝不碰日常 `web:3080`。

## 1. 严格执行顺序

1. 外部测试者完成当前工作树的自动回归和 `rp-dev` 手工验收。
...
1. 只读确认 `git status --short`，阅读本文和 `HANDOFF.md` §4；不清理当前 diff。
