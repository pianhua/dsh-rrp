...
> 勘察对象：已安装的 DSH 宿主 0.1.6-alpha.2（2026-09 快照；A5b 与 D12 的行号引用按此版本复核）。
> 目的：为 dsh-rrp 的「卡片展厅 / 状态看板 / 状态变更可见化 / 新建会话 / 右侧栏」寻找**原生宿主接缝**，不引入平行世界。
> 性质：**只读勘察**；除本文件外未修改、创建任何文件，未运行构建/测试。
> **版本口径**：本文件是宿主能力的**勘察证据**，当前基线与 seam 复核状态以 [`HOST_BASELINE.md`](HOST_BASELINE.md) 为准；两者不一致时先查基线。
...
- 版本核实（随基线 0.1.6-alpha.2 复核）：`WRK/dsh-client-ui-slots/package.json` 的 `version` = `0.1.6-alpha.2`；`dsh-client-ui-renderer` 随宿主安装树同版。
...
### A5b. 宿主 Session 对象的真实字段形状（issue #36 血泪，2026-09-20 补记）

来源：`dsh-api-session-controller/lib/typert.host.js` 内嵌的 typert 声明（宿主 0.1.6-alpha.2）。**结构性本地接口 + 全可选链无法被 TS 拦住**，读错字段只会静默回退默认值——以下为唯一权威形状：

```ts
class Session {
  readonly header: SessionHeader
  readonly inheritedEventCount: SessionLogOffset  // ← 顶层属性！不在 header 里
  readonly firstLiveSeq: SessionLogOffset
  // eventAt / snapshotEvents / ownEvents / isOwnSeq / seq / append / ...
}
interface SessionHeader {
  ...
  readonly cwd?: string
  readonly parentSession?: SessionId  // ← 直接在 header 上，没有 meta 层！
  readonly isSeeded: boolean          // fork 继承前缀标记
  readonly origin?: 'subagent'        // ← 子代理会话也带 parentSession，谱系消费必须过滤
  readonly delegationDepth?: number
  readonly agentPreset?: string
}
```

教训两则：① 世界线折叠曾读 `header.inheritedEventCount` / `header.meta.parentSession`（均为 undefined），分支全成 root 且测试与实现一起错——测试 Fake 必须镜像本表；② `conversation.view` slot 在宿主 app 包（dsh-client-ui-conversation/chat）中声明，**不在插件 node_modules 的子集里**，查 slot 时以本文件 A2 表 + 全局安装的宿主为准。
