...
| **剧情脉络** | **Summarizer Agent** | 独立智体，按轮次提炼高维、较长线的剧情罗盘（宏观目标、跨阶段矛盾、重大转折、长期伏笔），帮助叙事保持整体世界观；不代替 WorldState 维护近期活跃目标与矛盾，可关闭 | `总结Agent`、`剧情脉络官`、`编年官`（旧 UI 词）、`Chronicle`（英文界面） |
...
| 标准名 | 英文 | 定义 | 🚫 禁用 |
| :--- | :--- | :--- | :--- |
| **世界状态** | **WorldState** | 当前世界与人物的事实及近期可行动状态；包含近期目标、活跃矛盾、角色认知和仍有用的当前事件，不承载剧情脉络的高维长线视角 | `状态表`（作为标识符）、`state_table` |
| **角色通用字段** | **Common Character Field** | WorldState v1 明确支持的最小角色字段：名字、在场、穿搭、当前情感状态、好感、外貌；除名字作为身份键外均可缺省，缺省表示尚未追踪或不适用，不自动填默认值。好感底层为可选 number，范围与显示由卡包声明；未获确认的其他维度不得自行加入通用词汇 | 擅自增加未确认的通用字段 |
| **卡包专属状态字段** | **Card-defined State Field** | 由具体卡包声明、只在适用卡包中有意义的角色/物品/场景等扩展字段；字段应使用统一底层类型、展示和编辑机制，但语义由卡包定义，不得反向升级为全局通用字段 | `全局好感度`、`通用 affinity` |
| **近期目标** | **Current Objective** | WorldState 中当前仍可行动的目标；记录一名或多名负责者、预期结果、进度/状态与下一步。可标记一项主目标并排序其余目标；状态为待行动、进行中、受阻、已完成或已放弃；完成/放弃后关闭并留在历史中 | — |
| **活跃矛盾** | **Active Conflict** | 仍影响当前行动的矛盾；记录相关方、利害关系、简短压力描述与状态。状态为进行中、受控、已解决或已放弃；暂未提及不自动关闭，解决/放弃后关闭并留在历史中 | — |
| **角色认知条目** | **Character Cognition Entry** | 以角色和简短命题标识一条认知；可同时带有明确知道、相信但未证实、怀疑、听闻传言或明确误解等状态标记，但同一命题的明确知道与明确误解不能并存。新证据更新同一条目；无条目表示尚未追踪，不代表明确不知道；认知不改写客观世界状态 | — |
| **双向关系条目** | **Relationship Entry** | 一条记录表示一对角色，含共同关系标签及 A→B、B→A 各自的简短态度；未追踪某方向时不推定态度对称，可选记录有明确含义的数值 | — |
| **当前事件条目** | **Current World Event Entry** | 记录仍有用的承诺、秘密、发现等具体事项及类型、简短事实、相关角色和当前状态；解决后从当前列表关闭并保留在状态时间线中，不是完整叙事日志 | — |
| **追踪对象** | **Tracked Object** | 被 WorldState 长期维护的角色、群体/势力、物品或场景/地点；只有成为追踪对象后才拥有独立详情字段。可自动提出/建立，也可由玩家手动追踪；离场不等于删除 | — |
| **对象外部引用** | **Object External Reference** | 目标、矛盾或事件对尚未追踪对象的名称引用；之后可升级为追踪对象，不自动复制成两个对象 | — |
| **对象级动态字段** | **Object-scoped Dynamic Field** | 附着于具体追踪对象的自定义标量属性；同名字段在同一对象类型中语义/类型一致，各对象的值独立；全局事实留在全局作用域。重构目标类型为 number/string/boolean，暂不设数量上限 | — |
| **状态时间线** | **WorldState Timeline** | 持久记录每次实际 WorldState 变更；一次保存/推演批次为一条分组事件，含修改者、回合/现实时间及受影响字段的旧值/新值。仅新建会话开始记录，fork 继承分叉点前历史；只读，不提供旧状态回滚或历史点读档；支持按回合、修改者、对象/字段和变更类型筛选 | — |
| **归档对象** | **Archived Object** | 从当前默认列表隐藏但保留历史、关联引用并可恢复的追踪对象；归档不是删除 | — |
| **玩家矫正** | **Player Correction** | 玩家在 AI 更新后、下一轮前手动修正状态；**不是锁** | `玩家仲裁`、`Player Arbitration`、`Unlock`、`硬锁` |
| **状态推演** | **State Inference** | Chronicler 根据剧情明确证据判断「什么该记录、什么该更正」的过程 | `事实提取`、`ChangeSet 提取` |
