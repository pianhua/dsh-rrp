...
| :--- | :--- | :--- |
| `ctx.sessionProjections`（含 wire-less 单元的 register 重载） | `src/index.ts` 注册 6 个投影单元 | 未变化 |
| `session.append` + `user/message` 的 `source.rrp` 寄生 | `state-publisher.ts` 等 | 未变化；surface 事件必须带 `surfaceOp`（我们全部 append） |
| `ctx.jobs`（attachController + start） | `chronicler.ts` / `summarizer.ts` / `sediment-route.ts` | 仅内部改名，公开面未变 |
| `ctx.llm.stream` | 三个推演智能体 | 未变化 |
| `agent/created` / `agent/disposed` 生命周期 | `sediment-runtime.ts` / `index.ts` | `agent/session-start` 已废弃改为此名（我们本就使用新名） |
| `ctx.webServer` 路由注册 | correction / cards / activity / sediment / start | 未变化 |
...
| skills（scoped catalog） | `sediment-provider.ts` / `preset.ts` | 仅 `path` 字段上移到基接口 |
...
- `sidebar.right.pane.tab` 声明 `scope: 'session'` → `inject(sessionId)` ——世界状态/典籍页签按此消费。
...
**本仓库已清账（2026-09-18）**：新增 `rrpTranscript` 投影单元（`src/projection/transcript.ts`），Chronicler 三个转录读法、`state-publisher.adopt()`、`sediment-runtime.hasSedimentEvent` 全部改读投影切片，生产代码零 `snapshotEvents` 残留（`grep -rn snapshotEvents src/` 应为空）。
...
6. 更新本表「当前基线」与日期；把复核结论追加到 [`COMPLETED_WORK.md`](COMPLETED_WORK.md)。
