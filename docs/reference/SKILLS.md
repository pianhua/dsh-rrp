...
D8 要求「可以有动态沉淀，但不能过于激烈导致世界观崩塌」。约定：

1. **默认关闭**：动态沉淀不是默认行为；
2. **只在明确触发时发生**（例如玩家确认「记下这个设定」）；
3. **只新增，不覆写**既有 skill —— 修改既有世界观必须由玩家显式操作；
4. **产出可审阅**：沉淀结果应让玩家能看到、能删；
5. **单次体量受限**：一次沉淀只写一个主题，不做批量重写。

> ⚠️ 该能力**不进入最小版本**；实现前必须先在此处细化控制策略并回报所有者。
