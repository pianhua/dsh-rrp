...
> **背景**：决策 D7 —— 世界设定集 / 酒馆世界书（Lorebook）这一层，**改用 Skills 实现**。  
...
| 可审阅 | 草稿预览 + 「设定集」列表 + 删除；投影条目保持标准 Skill 的 name/description/content 结构 |
