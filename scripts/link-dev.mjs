...
if (!pkg.dsh?.bundle?.patch) throw new Error(`${pkg.name} declares no dsh.bundle.patch; run 'pnpm run build' first`)
if (!existsSync(manifestPath)) throw new Error(`profile ${profileName} not found at ${profileDir}; create it first`)
if (!existsSync(join(repoRoot, 'lib', 'index.js'))) throw new Error('lib/index.js missing — run "pnpm run build" first')
