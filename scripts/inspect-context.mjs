...
function decode(path) {
  ...
  for (let i = 0; i + 4 <= buffer.length; i += 1) {
    if (
      ...
      continue
    try {
      parts.push(zstdDecompressSync(buffer.slice(i)).toString('utf8'))
    } catch {
      /* not a frame start */
    }
  }
...
const owned = events.filter(
  (event) => event.type === 'user/message' && /^【(当前卡包|世界状态)/.test(textOf(event)),
)
const card = owned.filter((event) => textOf(event).startsWith('【当前卡包'))
const facts = owned.filter((event) => textOf(event).startsWith('【世界状态'))
const replaces = owned.filter(
  (event) => event.surfaceOp !== undefined && typeof event.surfaceOp === 'object',
)
...
console.log(
  'dsh-rrp context messages ON SURFACE: card=' +
    countOnSurface(card) +
    '  facts=' +
    countOnSurface(facts),
)
...
for (const event of owned.slice(-8)) {
  const op =
    event.surfaceOp === undefined
      ? 'append(?log-only)'
      : typeof event.surfaceOp === 'string'
        ? event.surfaceOp
        : JSON.stringify(event.surfaceOp)
  console.log(
    '  ' +
      String(event.seq).padStart(5) +
      ' | ' +
      op.padEnd(42) +
      ' | ' +
      String(onSurface.has(event.seq)).padEnd(5) +
      ' | ' +
      JSON.stringify(textOf(event).slice(0, 28)),
  )
}
