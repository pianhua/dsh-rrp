...
export { reservedNames, stageLoreDraft } from './lore-application.ts'
...
export function registerLoreRoute(ctx: Context): void {
  ...
  const faces: LoreApplicationFaces | undefined =
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (action === 'draft') {
            ...
            scheduleLoreDraft(faces, session, topic)
  ...
  ctx.effect(() => {
    const dispose = commands.register({
      ...
      handler: (invocation) => {
        ...
        scheduleLoreDraft(faces, session, invocation.rawInput.trim())
