...
describe('steward proposals staging', () => {
  it('stages, lists, and discards proposals per session', () => {
    const staged = stageProposal('s1', { kind: 'card-edit', card: 'test-card', file: 'card.md', content: '# 新', reason: 'r' })
    ...
    expect(list[0]).toMatchObject({ id: staged.id, kind: 'card-edit', card: 'test-card', file: 'card.md', reason: 'r' })
  ...
  it('keeps at most 20 proposals per session, dropping the oldest', () => {
    ...
    for (let index = 0; index < 21; index += 1) {
      const staged = stageProposal('s-cap', { kind: 'doc-note', title: 'n' + String(index), body: 'b' })
...
describe('steward proposals confirm (card-edit)', () => {
  it('writes the file into the user card dir, archives the old content, and consumes the proposal', () => {
    ...
    const staged = stageProposal('s1', {
      kind: 'card-edit', card: 'test-card', file: 'notes/extra.md', content: '# 补充设定\n新内容。',
    })
    ...
    const written = readFileSync(join(home, '.dsh-rrp', 'cards', 'test-card', 'notes', 'extra.md'), 'utf8')
  ...
  it('refuses shipped packs as read-only', () => {
    ...
    const home = tempHomeWithCard()
    const staged = stageProposal('s1', { kind: 'card-edit', card: 'yanmen-inn', file: 'card.md', content: '# x' })
    const result = confirmProposal('s1', staged.id, home)
  ...
  it('refuses unknown card packs', () => {
    const home = tempHomeWithCard()
    const staged = stageProposal('s1', { kind: 'card-edit', card: 'no-such-card', file: 'card.md', content: '# x' })
    const result = confirmProposal('s1', staged.id, home)
  ...
  it('refuses path escapes, absolute paths, and non-whitelisted extensions', () => {
    const home = tempHomeWithCard()
    const escape = stageProposal('s1', { kind: 'card-edit', card: 'test-card', file: '../outside.md', content: 'x' })
    const escaped = confirmProposal('s1', escape.id, home)
    expect(escaped.ok).toBe(false)
    if (!escaped.ok) expect(escaped.error).toContain('越界')

    const absolute = stageProposal('s1', { kind: 'card-edit', card: 'test-card', file: '/etc/passwd.md', content: 'x' })
    const refused = confirmProposal('s1', absolute.id, home)
    expect(refused.ok).toBe(false)
    if (!refused.ok) expect(refused.error).toContain('绝对路径')

    const exe = stageProposal('s1', { kind: 'card-edit', card: 'test-card', file: 'payload.exe', content: 'x' })
  ...
  it('reports unknown proposal ids', () => {
    const home = tempHomeWithCard()
    expect(confirmProposal('s1', 'nope', home)).toEqual({ ok: false, error: '提案不存在或已被处理' })
  })
})
