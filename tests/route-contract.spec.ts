...
function fakeHost(preset = 'rp-demo') {
  ...
  const projections = { stateOf: (_s: unknown, key: string) => (key === 'agentPreset' ? preset : undefined) }
  const webServer = {
  ...
  const ctx = {
    ...
    get: (name: string) => ({ webServer, sessions, sessionProjections: projections, agents: { get: () => undefined }, llm: { stream: () => [] }, jobs: { start: () => 'j1' } } as Record<string, unknown>)[name],
  }
...
describe('route contract (issue #22)', () => {
  ...
  it('every non-2xx response carries the contracted RrpErrorBody shape', async () => {
    ...
    const cases: Array<{ path: string; init: { method: string; body?: unknown; url?: string } }> = [
      ...
      { path: RRP_ROUTES.activity, init: { method: 'GET', url: 'http://localhost' + RRP_ROUTES.activity } },
      // 400: missing card id.
      { path: RRP_ROUTES.cardOne, init: { method: 'GET', url: 'http://localhost' + RRP_ROUTES.cardOne } },
    ]
  ...
  it('404 on an unknown session is a contract error body too', async () => {
    const host = fakeHost()
    registerCopilotRoute(host.ctx as never)
    const res = await call(host, RRP_ROUTES.copilot, { method: 'GET', url: 'http://localhost' + RRP_ROUTES.copilot + '?sessionId=nope' })
  ...
  it('SSE codec round-trips a full turn: chunks, action, terminal done', () => {
    const wire
      ...
      + encodeSseFrame(COPILOT_SSE.done, { turn: { role: 'copilot', text: '你好', at: 't' }, undoCount: 1 })
    const events: Array<{ event: string; data: unknown }> = []
    const rest = drainSse(wire, (event, data) => { events.push({ event, data }) })
    expect(rest).toBe('')
    expect(events.map((entry) => entry.event)).toEqual(['chunk', 'chunk', 'action', 'done'])
    expect(events.filter((entry) => entry.event === COPILOT_SSE.chunk).map((entry) => (entry.data as { text: string }).text).join('')).toBe('你好')
  })
  ...
  it('a frame split across network chunks is held until complete', () => {
    ...
    const seen: string[] = []
    const rest = drainSse(half, (event) => { seen.push(event) })
    expect(seen).toEqual([])
    drainSse(rest + tail, (event) => { seen.push(event) })
  ...
  it('malformed frames are skipped without sinking the stream', () => {
    ...
    const good = encodeSseFrame(COPILOT_SSE.done, { turn: { role: 'copilot', text: '', at: 't' }, undoCount: 0 })
    const seen: Array<{ event: string; data: unknown }> = []
    const rest = drainSse(broken + missing + good, (event, data) => { seen.push({ event, data }) })
    expect(rest).toBe('')
